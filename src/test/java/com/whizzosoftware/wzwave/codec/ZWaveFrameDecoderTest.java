...
public class ZWaveFrameDecoderTest {
    ...
    @Before
    public void before() {
        ctx = new MockChannelHandlerContext();
    }
    ...
    public void testPartialMessage() throws Exception {
        ...
        in = wrappedBuffer(in, wrappedBuffer(new byte[] {0x15,0x5A,0x2D,0x57,0x61,0x76,0x65,0x20,0x32,0x2E,0x37,0x38,0x00,0x01,(byte)0x9B}));
        assertEquals(18, in.readableBytes());
        decoder.callDecode(ctx, in, out);
    ...
    public void testOneFrameAcrossTwoReads() throws Exception {
        ...
        decoder.callDecode(ctx, buf, out);
        assertEquals(10, buf.readableBytes());
        assertEquals(0, out.size());

        buf = wrappedBuffer(buf, wrappedBuffer(new byte[] {0x20, 0x32, 0x2e, 0x37, 0x38, 0x00, 0x01, (byte)0x9b}));
        assertEquals(10 + 8, buf.readableBytes());
        decoder.callDecode(ctx, buf, out);
    ...
    public void testTwoFramesAcrossTwoReads() throws Exception {
        ...
        assertEquals(1, out.size());
        in = wrappedBuffer(in, wrappedBuffer(new byte[] {0x00, 0x04, 0x00, 0x0e, 0x07, (byte)0x86, 0x12, 0x06, 0x03, 0x28, 0x03, 0x19, 0x5c}));
        assertEquals(2 + 13, in.readableBytes());
        decoder.callDecode(ctx, in, out);
        assertEquals(0, in.readableBytes());
        assertEquals(2, out.size());
        assertTrue(out.get(0) instanceof ApplicationCommand);
    ...
    public void testFuncCallback() throws Exception {
        ZWaveFrameDecoder decoder = new ZWaveFrameDecoder();
        List<Object> out = new ArrayList<Object>();
        decoder.callDecode(ctx, wrappedBuffer(new byte[] {0x01, 0x09, 0x00, 0x13, 0x06, 0x02, 0x25, 0x02, 0x05, 0x01, (byte)0xC2}), out);
        decoder.callDecode(ctx, wrappedBuffer(new byte[] {0x01, 0x04, 0x01, 0x13, 0x01, (byte)0xE8}), out);
        decoder.callDecode(ctx, wrappedBuffer(new byte[] {0x01, 0x05, 0x00, 0x13, 0x01, 0x00, (byte)0xE8}), out);
    ...
    public void testIncompleteWithMultipleAttempts() throws Exception {
        ZWaveFrameDecoder decoder = new ZWaveFrameDecoder();
        List<Object> out = new ArrayList<>();
        ByteBuf in = wrappedBuffer(new byte[] {0x06, 0x01, 0x08, 0x01, 0x20, (byte)0xD3, 0x3C, (byte)0xB4, 0x11});
        decoder.callDecode(ctx, in, out);
        assertEquals(1, out.size());
        in = wrappedBuffer(in, wrappedBuffer(new byte[] {0x01}));
        decoder.callDecode(ctx, in, out);
        assertEquals(1, out.size());
        in = wrappedBuffer(in, wrappedBuffer(new byte[] {(byte)0x9D, 0x01}));
        decoder.callDecode(ctx, in, out);
        assertEquals(2, out.size());
        assertTrue(out.get(0) instanceof ACK);
        assertTrue(out.get(1) instanceof MemoryGetId);
        out.clear();
        in = wrappedBuffer(in, wrappedBuffer(new byte[] {0x05, 0x00, 0x13, 0x01, 0x00, (byte)0xE8}));
        decoder.callDecode(ctx, in, out);
        assertEquals(1, out.size());
        assertTrue(out.get(0) instanceof SendData);
    }
    ...
    public void testIncompleteWithMultipleAttempts2() throws Exception {
        ...
        buf = wrappedBuffer(buf, wrappedBuffer(new byte[] {0x01, 0x25, 0x01}));
        ...
        buf = wrappedBuffer(buf, wrappedBuffer(new byte[] {0x02, 0x05, 0x00, 0x1D, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}));
        ...
        buf = wrappedBuffer(buf, wrappedBuffer(new byte[] {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0x00, (byte)0xC3}));
        ...
        out = new ArrayList<>();
        decoder.callDecode(ctx, wrappedBuffer(new byte[] {0x06, 0x01, 0x09, 0x01, 0x41, (byte)0x93, 0x16, 0x01, 0x02, 0x02, 0x01, 0x33}), out);
        assertEquals(2, out.size());
        assertTrue(out.get(0) instanceof ACK);
    ...
    public void testInvalidChecksum() throws Exception {
        ZWaveFrameDecoder decoder = new ZWaveFrameDecoder();
        List<Object> out = new ArrayList<Object>();
        ByteBuf in = wrappedBuffer(new byte[] {0x01, 0x04, 0x01, 0x13, 0x01, (byte)0xe9});
        try {
            decoder.callDecode(ctx, in, out);
        }
        finally {
    ...
    public void testLastEmpty() throws Exception {
        ZWaveFrameDecoder decoder = new ZWaveFrameDecoder();
        List<Object> out = new ArrayList<Object>();
        ByteBuf in = wrappedBuffer(new byte[] {});
        decoder.decodeLast(ctx, in, out);
        assertEquals(0, out.size());
    }
}
